...
class Borders {
  public:
    ...
    void set_outside(const Border& border) { top = left = bottom = right = border; }

    // Set inside borders only
    void set_inside(const Border& border) { inside_horizontal = inside_vertical = border; }
    ...
    bool has_visible() const {
        return top.is_visible() || left.is_visible() || bottom.is_visible() || right.is_visible() ||
...
class Shading {
  public:
    ...
    bool has_fill() const { return !background.is_auto(); }
...
class CellFormat {
  public:
    ...
    // Convenience methods
    // ------------------------------------------------------------------------
    void set_margins(double all) { top_margin = bottom_margin = left_margin = right_margin = all; }

    bool is_merged() const { return vertical_merge || horizontal_merge > 1; }

    bool is_vertical_merge_start() const { return vertical_merge && vertical_merge_start; }

    bool is_vertical_merge_continue() const { return vertical_merge && !vertical_merge_start; }
};
