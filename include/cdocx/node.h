...
class NodeCollection {
  ...
  public:
    ...
    std::shared_ptr<Node> operator[](size_t index) const { return get_item(index); }
    std::shared_ptr<Node> first() const { return nodes_.empty() ? nullptr : nodes_.front(); }
    std::shared_ptr<Node> last() const { return nodes_.empty() ? nullptr : nodes_.back(); }
