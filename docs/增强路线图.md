...
| 版本 | 时间 | 主题 | 主要功能 | 当前状态 |
|-----|------|------|---------|---------|
| v0.7.0 | Q2 2026 | 核心增强 | 字段系统、超链接、样式改进 | 已完成 |
| v0.8.0 | Q2 2026 | 模板引擎 | TemplateEngine、BookmarkInserter、CommentCollection、MailMerge、Watermark、StyleCollection | **当前版本** |
| v0.9.0 | Q3 2026 | 导出功能 | PDF 导出、HTML 导出 | 规划中 |
| v1.0.0 | Q4 2026 | 专业功能 | 图表、文档比较、修订跟踪 | 规划中 |
...
#### 当前状态
- ✅ `Field` DOM 类及 `FieldType` 枚举已实现
- ✅ 字段开关 (switches) 定制及 DOM↔XML 双向同步已实现
- ✅ `DocumentBuilder::insert_page_number()` / `insert_date()` / `insert_time()` / `insert_merge_field()` / `insert_table_of_contents()` 已实现
- ✅ 字段克隆、unlink、locked/dirty 属性已实现
...
// 合并字段
builder.insert_merge_field("Name", "\\* Upper");

// 目录字段
builder.insert_table_of_contents("\\o \"1-3\" \\h \\z");

// 手动创建字段并配置开关
auto field = std::make_shared<cdocx::Field>(cdocx::FieldType::Page);
...
#### 当前状态
- ✅ `Paragraph::append_hyperlink()` 已实现
- ✅ `Hyperlink` DOM 类及序列化/反序列化已实现
- ✅ `DocumentBuilder::insert_hyperlink()` 已实现

#### 目标 API

```cpp
// 便捷插入超链接
paragraph.append_hyperlink("点击访问", "https://example.com");

// 书签链接
paragraph.append_hyperlink("跳转到第一章", "bookmark_chapter1",
                           HyperlinkType::Bookmark);

// 邮件链接
paragraph.append_hyperlink("联系我们", "mailto:info@example.com");

// 修改超链接
hyperlink.set_address("https://new-url.com");
hyperlink.set_result("新文本");
```
...
## v0.8.0 - 模板引擎与高级功能 (Template Engine & Advanced Features)

### 8.1 TemplateEngine 统一模板引擎

- 字典风格 API：`engine["key"] = value`
- 支持占位符（`{{key}}`）和书签双目标
- 文本/图片替换、格式策略（Preserve/Override/Custom）
- 作用域模式（First/All）与行为模式（Replace/Insert）
- 模板分析器工具自动生成 C++ 代码

### 8.2 BookmarkInserter 书签插入

- 在现有文本周围自动插入 `w:bookmarkStart` / `w:bookmarkEnd`
- 支持跨 Run 文本匹配与批量插入
- `insert_batch` 按文本长度降序排序，防止子串隐藏

### 8.3 批注与邮件合并

- `CommentCollection`：文档级批注增删查
- `MailMerge`：简单字段替换与清理选项

### 8.4 水印与样式

- `Watermark`：文本水印与图片水印，支持 header 绑定
- `StyleCollection`：样式增删查，支持 `get_by_name` / `get_by_style_id`

### 8.5 脚注/尾注集合 API

- `FootnoteCollection` / `EndnoteCollection`：文档级管理器
- 支持 add / remove / get_by_id，含 DOM↔XML 双向同步

### 8.6 字段 Switches 定制

- `DocumentBuilder::insert_page_number` / `insert_date` / `insert_time` / `insert_merge_field` / `insert_table_of_contents`
- `Field` 类支持 switches 增删查与 DOM↔XML 往返

### 8.7 段落与节便捷方法

- `Paragraph::append_hyperlink(text, url)` / `append_hyperlink(text, bookmark, true)`
- `Paragraph::append_page_number(switches)` / `append_date(switches)` / `append_time(switches)` / `append_merge_field(name, switches)`
- `Section::ensure_header()` / `ensure_footer()` — get if exists, otherwise create

---

## v0.9.0 - 导出功能 (Export Capabilities)

### 9.1 PDF 导出 (PDF Export)
...
// 配置内部段落
textbox->paragraphs().add_run("更多文本", cdocx::kBold);
```

### 10.2 图片效果 (Image Effects)
