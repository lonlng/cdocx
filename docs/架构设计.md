...
public:
    ...
    CompositeNode* get_parent() const;
    std::shared_ptr<Node> get_previous_sibling() const;
    std::shared_ptr<Node> get_next_sibling() const;
    std::shared_ptr<Node> get_previous_node_in_document() const;  // 前序遍历上一个节点
    std::shared_ptr<Node> get_next_node_in_document() const;      // 前序遍历下一个节点
...
- **`clone()` 深拷贝**：所有主要 DOM 节点（`Document`、`Section`、`Body`、`Paragraph`、`Table`、`Row`、`Cell`、`HeaderFooter`、`Run`、`BookmarkStart/End`、`Hyperlink`、`Field`、`SpecialChar`）均已实现 `clone(bool deep)`，支持完整子节点递归复制
- **文档级遍历**：`Node::get_previous_node_in_document()`、`get_next_node_in_document()`、`get_previous_logical()`、`get_next_logical()` 已实现前序遍历
- **DOM ↔ XML 同步**：已完成 `Run`、`Paragraph`、`Table`、`Section`、`Body`、`Bookmark`、`Hyperlink`、`Field`、`Footnote`、`Comment`、`Style` 的序列化与反序列化
- **HeaderFooter DOM 操作**：`append_paragraph()`、`append_table()`、`get_paragraphs()` 已实现
- **`TableOperations`**：列插入/删除、`auto_fit`、`merge_cells`、`split_cell` 已实现
- **`CDocx` 工厂类**：`create_document()`、`load_document()`、`save_document()` 已实现
- **`DocumentBuilder` Fluent API**：`insert_hyperlink()`、`insert_image()`、`move_to_bookmark()`、`insert_page_number()`、`insert_date()`、`insert_footnote()` 等已实现
- **`TemplateEngine`**：统一字典风格模板 API，支持占位符和书签替换，含格式策略和作用域控制
- **`CommentCollection`**、`MailMerge`**、**`Watermark`**、**`StyleCollection`** 均已实现
