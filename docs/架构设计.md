...
- **DOM ↔ XML 同步**：`document_sync.cpp` 已完成 `Run`、`Paragraph`、`Table`、`Section`、`Body`、`Bookmark`、`Hyperlink`、`Field` 的序列化与反序列化
- **HeaderFooter DOM 操作**：`append_paragraph()`、`append_table()`、`get_paragraphs()` 已实现

尚未实现：
- `TableOperations`（列插入/删除、`auto_fit`）
- `CDocx` 工厂类、`DocumentBuilder` Fluent API（`insert_hyperlink`、`insert_image`、`move_to_bookmark` 等）
