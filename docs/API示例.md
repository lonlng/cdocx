...
> ⚠️ **重要提示**：本文档展示的是 **v2.0 目标 API 设计**，其中部分 API 已在前序版本（v0.7.0）中实现，但仍有大量方法仅在头文件中声明、尚未有对应 `.cpp` 实现。实际可工作的 API 请以 [`AGENTS.md`](../AGENTS.md) 和源码为准。
>
> **已实现的 DOM API（v0.7.0）**：`Document::open/save/clone`、`append_section`、`get_body`、`append_paragraph`、`append_run`、`append_table`、`get_rows/get_cells`、`set_text`、`Section::add_header/footer/clone`、`Paragraph::clone`、`Table::clone`、`Row::clone`、`Cell::clone`、`BookmarkStart/End`、`Field`（序列化/反序列化）、`Hyperlink`（序列化/反序列化）、`Node::get_next_node_in_document` / `get_previous_node_in_document`、`HeaderFooter::append_paragraph` / `append_table` / `get_paragraphs`、基础 `Font`/`ParagraphFormat` 设置、`DocumentSearch::find` / `find_all` / `replace` / `replace_all` / `replace_with_formatting` / `find_and_process`。
>
> **尚未实现的 API**：`Document::create()`、`Document::load_from_file()`、`save(stream)`、`Paragraph::append_hyperlink()`（便捷方法）、`append_page_number()`、`append_date()`、`Section::ensure_header/footer()`、`Document::get_style()`/`add_style()` 等。
...
para->append_page_number();  // 注意：尚未实现
para->append_run(" of ");
para->append_num_pages();  // 注意：尚未实现
```
...
// 日期
para->append_date("yyyy-MM-dd");  // 注意：尚未实现

// 时间
para->append_run(" at ");
para->append_time("HH:mm:ss");  // 注意：尚未实现

// 创建日期 (文档属性)
para->append_run("\nDocument created: ");
para->append_field(cdocx::FieldType::CreateDate);  // 注意：尚未实现
```
...
// 网页链接
para->append_hyperlink("Visit Example.com", "https://example.com");  // 注意：尚未实现

// 书签链接
para->append_hyperlink("Go to Summary", "SummaryBookmark", true);  // 注意：尚未实现

// 格式化链接
auto hyperlink = para->append_hyperlink("Click here", "https://example.com");  // 注意：尚未实现
hyperlink->get_font()
...
para->append_field(cdocx::FieldType::Author);  // 注意：尚未实现

para->append_run("\nTitle: ");
para->append_field(cdocx::FieldType::Title);  // 注意：尚未实现

para->append_run("\nSubject: ");
para->append_field(cdocx::FieldType::Subject);  // 注意：尚未实现
```
...
// 添加页眉
auto header = section->ensure_header(cdocx::HeaderFooterType::Primary);  // 注意：尚未实现
header->append_paragraph("Header Text");

// 添加页脚
auto footer = section->ensure_footer(cdocx::HeaderFooterType::Primary);  // 注意：尚未实现
...
```cpp
// 获取或创建样式
auto style = doc->get_style("CustomHeading");  // 注意：尚未实现
if (!style) {
    style = std::make_shared<cdocx::Style>("CustomHeading", cdocx::StyleType::Paragraph);
    doc->add_style(style);  // 注意：尚未实现
}
...
int main() {
    ...
    // 设置文档属性
    doc->get_built_in_properties().title = "Sales Report";  // 注意：尚未实现
    doc->get_built_in_properties().author = "John Doe";  // 注意：尚未实现
    ...
    date_para->append_date("MMMM dd, yyyy");  // 注意：尚未实现
    ...
    // 添加页眉
    auto header = doc->get_first_section()->ensure_header(  // 注意：尚未实现
        cdocx::HeaderFooterType::Primary
    );
    ...
    // 添加页脚
    auto footer = doc->get_first_section()->ensure_footer(  // 注意：尚未实现
        cdocx::HeaderFooterType::Primary
    );
    ...
    footer_para->append_page_number();  // 注意：尚未实现
    footer_para->append_run(" of ");
    footer_para->append_num_pages();  // 注意：尚未实现
