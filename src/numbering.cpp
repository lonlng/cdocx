...
bool NumberingManager::override_level(NumberingId numId,
...
void NumberingManager::save_to_xml(pugi::xml_node numbering_root) {
    ...
    for (const auto& [id, def] : abstractDefinitions_) {
        ...
        abstractNum.append_attribute("w:abstractNumId").set_value(static_cast<unsigned int>(id));
        ...
        for (int i = 0; i < 9; ++i) {
            ...
            start.append_attribute("w:val").set_value(static_cast<unsigned int>(level.startNumber));
