...
public:
    ...
    private:
        ...
        /// Constructor.
        ASPOSECPP_SHARED_API TransitionTime(DateTime time_of_day, int month, int week, int day, DayOfWeek day_of_week, bool is_fixed_date_rule);

        /// Verify constructor arguments.
        static void VerifyTransitionTime(DateTime time_of_day, int month, int week, int day, DayOfWeek day_of_week);
    };
    ...
    private:
        ...
        /// Constructor.
        ASPOSECPP_SHARED_API AdjustmentRule(DateTime date_start, DateTime date_end, TimeSpan daylight_delta,
            const TransitionTime& daylight_transition_start, const TransitionTime& daylight_transition_end,
            TimeSpan base_utc_offset_delta, bool no_daylight_transitions);
        MEMBER_FUNCTION_MAKE_OBJECT(
            AdjustmentRule,
            CODEPORTING_ARGS(
                const DateTime& date_start,
                const DateTime& date_end,
                const TimeSpan& daylight_delta,
                const TransitionTime& daylight_transition_start,
                const TransitionTime& daylight_transition_end,
                const TimeSpan& base_utc_offset_delta,
                bool no_daylight_transitions
            ),
            CODEPORTING_ARGS(
                date_start,
                ...
                no_daylight_transitions
            )
        );

        /// Verify constructor arguments.
        static void VerifyAdjustmentRule(DateTime date_start, DateTime date_end, TimeSpan daylight_delta,
    ...
    ASPOSECPP_SHARED_API ArrayPtr<TimeSpan> GetAmbiguousTimeOffsets(DateTime date_time) const;
    /// Gets UTC dates and times that a specified date and time can be mapped to.
...
protected:
    ...
    /// @param icu_time_zone ICU Time zone.
    /// @param time_zone_kind Time zone kind (UTC, Local or Unspecified).
    TimeZoneInfo(
        const String& id,
    ...
    MEMBER_FUNCTION_MAKE_OBJECT(
        ...
        CODEPORTING_ARGS(
            ...
            daylight_display_name,
            adjustment_rules,
            disable_daylight_saving_time,
            std::forward<std::unique_ptr<const codeporting_icu::BasicTimeZone>&&>(icu_time_zone),
            time_zone_kind
        )
    );
