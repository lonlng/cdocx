...
template<typename T>
class ASPOSECPP_SHARED_CLASS IComparer : virtual public Object
{
    /// RTTI information.
...
template<class T>
struct ASPOSECPP_SHARED_CLASS ComparerAdapter
{
...
/// @tparam T Type being compared.
template<class T>
class ASPOSECPP_SHARED_CLASS DefaultComparer : public IComparer<T>
{
public:
...
template<class T>
class ASPOSECPP_SHARED_CLASS Comparer : public IComparer<T>
{
public:
    /// Interface implemented.
    using BaseType = IComparer<T>;
    /// Curent type.
    using ThisType = Comparer<T>;
...
public:
    ...
    /// @return An object that inherits System.Collections.Generic.Comparer and serves as a sort order comparer for type T.
    static SharedPtr<IComparer<T>> get_Default()
    {
        static SharedPtr<IComparer<T>> s_default_comparer = System::MakeObject<DefaultComparer<T>>();
        return s_default_comparer;
    }
};
