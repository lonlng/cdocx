...
/// Contains the date and time of day relative to Coordinated Universal Time.
...
private:
    ...
    {
        ThisTypeInfo()
            : TypeInfoPtr(u"System::DateTimeOffset")
        {}
    };
};
...
/// @return \p stream.
inline std::ostream& operator<<(std::ostream& stream, DateTimeOffset value)
{
...
/// @param value Data to insert.
/// @return \p stream.
inline std::wostream& operator<<(std::wostream& stream, DateTimeOffset value)
{
    stream << value.ToString();
    return stream;
}
