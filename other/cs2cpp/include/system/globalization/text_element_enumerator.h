    ...
    // , public Collections::IEnumerator (Not implemented)
{
    ...
    friend class StringInfo;

    /// Constructor.
    /// @param str String to iterate through.
    /// @param start_index Start index.
    /// @param use_string_aliasing Create read-only string alias instead of copying
    ASPOSECPP_SHARED_API TextElementEnumerator(const String& str, int start_index = 0, bool use_string_aliasing = false);
    MEMBER_FUNCTION_MAKE_OBJECT(TextElementEnumerator, CODEPORTING_ARGS(const String& str, int start_index = 0, bool use_string_aliasing = false), CODEPORTING_ARGS(str, start_index, use_string_aliasing));
...
private:
    ...
    /// Iterator object.
    std::unique_ptr<codeporting_icu::BreakIterator> m_break_iterator;
