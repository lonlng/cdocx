...
public:
    ...
    virtual ASPOSECPP_SHARED_API DayOfWeek GetDayOfWeek(DateTime time) const;
...
private:
    ...
    const std::unique_ptr<codeporting_icu::Calendar> m_calendar;
    ...
    codeporting_icu::Calendar& GetCalendar(const LockContext&) const;
    ...
    void SetDateTime(DateTime time, const LockContext&) const;
    /// Sets date held by calendar.
    /// @param time Time point to assign to the calendar.
    ...
    int GetDateTimeField(DateTime time, int field, const char16_t* field_name) const;
    ...
    /// @param field_name Name of the field.
    /// @return Time-point after calculation is done.
    DateTime AddValueToDateTimeField(DateTime time, int field, int value, const char16_t* field_name) const;
