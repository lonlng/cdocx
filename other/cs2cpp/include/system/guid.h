...
public:
    ...
    /// @param g The string representation of a GUID to be represented by the object being constructed
    ...
    /// @param b Bits 32-47 of the GUID
    /// @param c Bits 48-63 of the GUID
    /// @param d A byte array view containing bits 64-127 of the GUID
    ...
    /// @returns A Guid object representing a GUID equivalent to that represented by @p input
    static ASPOSECPP_SHARED_API Guid Parse(const String& input);
    /// Tries to convert the specified string into Guid object.
    /// @param input The string to convert
    /// @param g Output Guid object, if successful.
...
private:
    ...
    /// The utility structures for Natvis.
    struct NatvisHelpers
    {
        /// The utility structure for Natvis.
        struct Hex { unsigned char c; };
        /// The utility structure for Natvis.
        struct HexLow { unsigned char c; };
        /// The utility structure for Natvis.
        struct HexHigh { unsigned char c; };
    };
};
...
/// @param guid Data to insert.
/// @return \p stream.
inline std::wostream& operator<<(std::wostream& stream, const Guid& guid)
{
    stream << guid.ToString();
    return stream;
}
