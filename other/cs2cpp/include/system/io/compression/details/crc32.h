...
public:
    ...
    /// @param offset Offset inside array given.
    /// @param count Number of bytes to be read.
    ASPOSECPP_SHARED_API void SlurpBlock(const System::Details::ArrayView<uint8_t>& block, int offset, int count);
    /// Update the value for the running CRC32 using the given block of bytes.
    /// This is useful when using the CRC32() class in a Stream.
    /// @param block Bytes to calculate CRC for.
    /// @param offset Offset inside array given.
    ...
    void SlurpBlock(const System::Details::StackArray<uint8_t, N> &block, int offset, int count)
    {
        SlurpBlock(static_cast<System::Details::ArrayView<uint8_t>>(block), offset, count);
    }
