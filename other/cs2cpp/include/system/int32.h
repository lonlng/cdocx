...
public:
    ...
    // Span functions
    static ASPOSECPP_SHARED_API int32_t Parse(const ReadOnlySpan<char16_t>& span);
    static ASPOSECPP_SHARED_API int32_t Parse(const ReadOnlySpan<char16_t> &span, std::nullptr_t) { return Parse(span); }
    static ASPOSECPP_SHARED_API int32_t Parse(const ReadOnlySpan<char16_t> &span, Globalization::NumberStyles styles, const SharedPtr<IFormatProvider>& provider);
