...
public:
    ...
    /// Releases all pointers to objects without disposing them. Should be called to make silent exit from using block
    void Release()
    {
        m_objects.fill(nullptr);
    }
...
#ifndef USING_STATEMENT_BEHAVIOR_START
#define USING_STATEMENT_BEHAVIOR_START(...)\
{\
    System::Details::DisposeGuard<std::tuple_size<decltype(std::make_tuple(__VA_ARGS__))>::value> __dispose_guard({ __VA_ARGS__ });\
    try\
    {

#define USING_STATEMENT_BEHAVIOR_END()\
    }\
    catch(...)\
    {\
        __dispose_guard.SetCurrentException(std::current_exception());\
    }\
}
#endif //USING_START
