...
public:
    ...
    ASPOSECPP_SHARED_API GroupCollection(const WeakPtr<Match>& match);

    /// Disables adding element into collection.
    /// @param item Item to add; ignored.
    void Add(const GroupPtr &item) override { ASPOSE_UNUSED(item); }
    /// Disables dropping elements from collection.
    void Clear() override {}
    /// Disables removing element from collection.
    /// @param item Item to remove; ignored.
    bool Remove(const GroupPtr& item) override { ASPOSE_UNUSED(item); return false; }
    ...
    ASPOSECPP_SHARED_API GroupPtr idx_get(int index) const override;
...
protected:
    /// Destructor.
    ASPOSECPP_SHARED_API ~GroupCollection() override;
