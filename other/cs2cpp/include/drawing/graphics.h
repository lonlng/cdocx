...
/// Forward declaration of SkClipStack class.
ASPOSECPP_3RD_PARTY_CLASS(SkClipStack);
/// Forward declaration of SkSize class.
ASPOSECPP_3RD_PARTY_STRUCT(SkSize);

namespace Aspose { namespace Testing { class GraphicsTest; } }
namespace Aspose { namespace Testing { namespace ReleaseAsDebug { class Test; } } }
namespace Aspose { namespace Images { namespace Pal { class GraphicsPal; } } }
...
public:
    ...
    /// Draws the specified region of the specified image at the specified location.
    /// @param image The image to draw
    /// @param destPoints An array view containing three points that define a parallelogram on the drawing surface to draw the image to
    /// @param srcRect A rectangle that defines the region of the specified image to draw
    /// @param srcUnit The measurement units used by @p srcRect parameter
    /// @param imgAttributes Specifies coloring and gamma information for the image
    ASPOSECPP_SHARED_API void DrawImage(const SharedPtr<Image>& image, const System::Details::ArrayView<PointF>& destPoints,
                                        const RectangleF& srcRect, GraphicsUnit srcUnit,
                                        const Imaging::ImageAttributesPtr& imgAttributes);
    /// Draws the specified region of the specified image at the specified location.
    /// @param image The image to draw
    /// @param destPoints An stack array containing three points that define a parallelogram on the drawing surface to draw the image to
    /// @param srcRect A rectangle that defines the region of the specified image to draw
    /// @param srcUnit The measurement units used by @p srcRect parameter
    /// @param imgAttributes Specifies coloring and gamma information for the image
    template<std::size_t N>
    void DrawImage(const SharedPtr<Image>& image, const System::Details::StackArray<PointF, N>& destPoints,
                   const RectangleF& srcRect, GraphicsUnit srcUnit,
                   const Imaging::ImageAttributesPtr& imgAttributes)
    {
        DrawImage(image, static_cast<System::Details::ArrayView<PointF>>(destPoints), srcRect, srcUnit, imgAttributes);
    }
    ...
    /// Draws a specified image using its original physical size at a specified location.
    /// @param image The image to draw
    /// @param rect The rectangle that specifies the upper-left corner of the drawn image. The X and Y properties of the rectangle specify the upper-left corner.
    /// The width and height values are ignored.
    ASPOSECPP_SHARED_API void DrawImageUnscaled(const SharedPtr<Image>& image, const Rectangle& rect);
    /// Draws a specified image using its original physical size at a specified location.
    /// @param image The image to draw
    /// @param point The Point structure that specifies the upper-left corner of the drawn image.
...
private:
    ...
    std::unique_ptr<SkClipStack> m_sk_clip_stack;

    friend class Aspose::Testing::GraphicsTest;
    friend class Aspose::Testing::ReleaseAsDebug::Test;
    friend class Aspose::Images::Pal::GraphicsPal;
    ...
    ASPOSECPP_SHARED_API Graphics(std::unique_ptr<SkCanvas> canvas, float hRes = 96.0f, float vRes = 96.0f);
    ASPOSECPP_SHARED_API MEMBER_FUNCTION_MAKE_OBJECT_DECLARATION(
        Graphics,
        CODEPORTING_ARGS(std::unique_ptr<SkCanvas> canvas, float hRes = 96.0f, float vRes = 96.0f)
    );
    ...
    SharedPtr<Drawing2D::Matrix> CalculateWorldMatrix(bool apply_shift = false, bool calc_scale = true) const;
    SharedPtr<Drawing2D::Matrix> CalculateBaseMatrix() const;
    PointF CalculateWorldScale() const;
    ...
    /// @param layout The layout size
    /// @param font_size Font size
    /// @param format The string format
    /// @returns A SizeF object that represents corrected text size
    static SizeF CorrectSize(const SizeF& size, const PointF& scale, const SizeF& layout, float font_size,
                             const System::SharedPtr<StringFormat>& format);
