...
public:
    ...
    /// @returns True if the Color was created using either the FromName(String) method or the FromKnownColor(KnownColor) method; otherwise, it returns false.
...
public:
    ...
    /// Returns the Hue-Saturation-Brightness (HSB) hue value, in degrees, for the color reprsented by the current
    /// object.
    ASPOSECPP_SHARED_API float GetHue();
    /// Returns the Hue-Saturation-Brightness (HSB) saturation for the color reprsented by the current object.
    ASPOSECPP_SHARED_API float GetSaturation();
    ...
    /// Always returns false.
    bool operator==(const std::nullptr_t &r) const { ASPOSE_UNUSED(r); return false; };
    /// Always returns true.
    bool operator!=(const std::nullptr_t &r) const { ASPOSE_UNUSED(r); return true; };
...
/// @param color Data to insert.
/// @return \p stream.
inline std::wostream& operator<<(std::wostream& stream, Color color)
{
    stream << color.ToString();
    return stream;
}
