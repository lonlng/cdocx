...
namespace Detail {
    ...
    ASPOSECPP_3RD_PARTY_ENUM_HOLDER(SkEncodedImageFormat, int, SkEncodedFormatHolder);
}
...
private:
    ...
    /// Creates a palette by the pixel format.
    /// @param pixel_format to create a palette.
    static sk_sp<SkColorTable> CreatePalette(Imaging::PixelFormat pixel_format);
    /// Returns a palette from m_sk_bitmap if it doesn't set then creates a palette by the pixel format.
    /// @param pixel_format to select a palette.
    sk_sp<SkColorTable> ChoosePalette(Imaging::PixelFormat pixel_format) const;
