...
- Namespace: `cdocx`
- Version: 0.2
- External Dependencies: pugixml (Git submodule), zip library (bundled)
...
samples/
├── sample_template.cpp      # Template usage example (currently enabled)
...
### Core Dependencies
- **pugixml**: XML parsing and manipulation (Git submodule)
- **zip**: ZIP archive handling for DOCX format (bundled)
- **C++17**: Modern C++ features

### Build System
- **CMake**: Cross-platform build configuration (minimum 3.0)
...
- **Python**: Test data generation scripts (3.6+)
...
- `BUILD_SAMPLES=ON/OFF` - Build sample programs (default: ON)
...
Test DOCX files are automatically generated by `generate_test_data.py` during CMake configuration. The script creates minimal DOCX files with proper ZIP structure and XML content for testing purposes.
...
4. Update test data generation in `generate_test_data.py` if needed
...
### Testing Issues
- Regenerate test data if files are missing: `python test/generate_test_data.py`
- Check Python version for test data generation (3.6+)
- Verify test file locations in build directory

## Continuous Integration

The project uses Travis CI with the following configuration:
- Ubuntu Bionic with GCC
- CMake build system
- Automated testing with CTest
- Build matrix includes basic build and test execution
